...
```mermaid
flowchart TB
    Client["Android 客户端<br/>文字输入 / 图片上传 / 商品卡片操作"]
    API["FastAPI 接口层<br/>REST / SSE / Multipart"]
...
**FastAPI 服务端**：对外暴露 REST 接口（商品列表/详情、购物车 CRUD、会话调试）和 SSE 流式接口（主对话 `/api/chat/stream`、图文上传 `/api/chat/stream/upload`）。请求到达后由路由层分发给 `ShoppingAgent`，响应以 SSE 事件流形式逐步返回。

**ShoppingAgent 编排引擎**：系统的核心，注入 23 个协作组件，覆盖输入预处理、意图解析、流程决策、检索调度、工具执行、回复生成、前端事件构造、历史持久化和画像刷新。各组件通过依赖注入协作，由中心 Agent 根据流程类型统一调度。
...
| ShoppingAgent | `app/agents/shopping_agent.py` | 中心编排入口，注入 23 个组件，串联全部处理环节 |
...
**第三阶段——生成**：`ResponseGenerationModule.generate()` 根据当前业务流程选择不同策略。购物车/结算类直接回传工具执行结果（不调用 LLM）；偏好更新/澄清/闲聊/越界使用本地模板；推荐/对比/场景方案等需要自然表达的流程，先构造本地模板作为兜底，再根据 ModelRouter 决策决定是否调用 LLM 进行润色或生成。每次 LLM 调用前后记录 `last_llm_called` 状态。需要说明的是，一轮请求中可能涉及多次 LLM 调用——IntentPlan 解析、回复生成、用户画像刷新、多模态视觉分析和购物车画像分析等环节均可独立触发 LLM 调用，但每次调用都在受约束的上下文中进行，且不依赖其他 LLM Agent 的自然语言输出作为输入。
...
**长期偏好记忆**维护用户跨会话的稳定购物偏好，持久化在 `storage/user_history/{user_id}/profile.json`，包含画像摘要、结构化画像（说话风格、价格偏好、品牌偏好、排斥条件、决策风格）和语义记忆（类目计数、功能约束计数、价格信号等统计）。长期偏好作为软约束影响推荐排序和话术风格，但绝不覆盖当前轮硬约束——这一点在 5.1 节中展开。
